...
describe("runMigrations", () => {
  ...
  test("fresh DB: creates _migrations, applies 0001_init.sql, all three tables exist", () => {
    ...
    expect(applied.map((r) => r.name)).toEqual([
      ...
      "0008_cursor_runs_created_at_ms.sql",
      "0009_driver_streams_tier.sql",
      "0010_driver_streams_provider.sql",
      "0012_driver_streams_continuation.sql",
      "0013_escalations.sql",
      "0014_driver_streams_review_cycles.sql",
      "0015_driver_review_artifacts.sql",
      "0016_driver_streams_rolls_up.sql",
    ...
    expect(columns).toContain("created_at_ms");

    const driverStreamColumns = db
      .prepare("PRAGMA table_info(driver_streams)")
      .all()
      .map((r) => (r as { name: string }).name);
    expect(driverStreamColumns).toContain("provider");
    expect(driverStreamColumns).toContain("review_cycles");
    expect(driverStreamColumns).toContain("rolls_up");
    expect(driverStreamColumns).toContain("model_id");
    ...
    const workflowRunColumns = db
      ...
      .all()
      .map((r) => (r as { name: string }).name);
    expect(workflowRunColumns).toContain("last_event_at");
  });
