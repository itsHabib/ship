...
- **`LocalClaudeRunner`** — drives `query({ prompt, options })` with local Claude credential support (`CLAUDE_CODE_OAUTH_TOKEN`, `ANTHROPIC_AUTH_TOKEN`, or `ANTHROPIC_API_KEY`) and gateway env injection; unattended runs always request SDK `auto` permission mode, verify the live mode reported by `system/init`, and fail closed on a missing or mismatched report; rejects non-local runtime; `attach` throws `OperationNotSupportedError`.
