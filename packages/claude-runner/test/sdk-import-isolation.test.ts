/**
 * SDK isolation: `@anthropic-ai/claude-agent-sdk` is imported in exactly one package.
 */
...
describe("SDK isolation — @anthropic-ai/claude-agent-sdk is imported in @ship/claude-runner only", () => {
  test("no other package imports from @anthropic-ai/claude-agent-sdk", () => {
    ...
    if (hits.length > 0) {
      ...
      throw new Error(
        `SDK isolation violated: ${String(hits.length)} import(s) of @anthropic-ai/claude-agent-sdk found outside @ship/claude-runner:\n${detail}`,
      );
    }
    expect(hits).toEqual([]);
  });
