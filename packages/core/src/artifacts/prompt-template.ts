...
export function renderImplementationPrompt(input: RenderImplementationPromptInput): string {
  ...
  return [
    ...
    "   - `task` with subagent_type `code-reviewer` — pass the diff for a P0/P1/P2/P3 review against the repo's conventions (correctness, security, edge cases, design).",
    "   - `task` with subagent_type `naming-critic` — pass the diff for a naming-specialty review (no Impl suffix, no And/Or in names, no generic package names like `shared`/`utils`/`common`, no JSDoc). Complements `code-reviewer`; both should fire when new symbols were added or renamed.",
    "   - `task` with subagent_type `scope-tracker` — verify the diff stays inside the task doc's Scope section.",
    "   - `task` with subagent_type `test-author` — fire whenever the diff adds a new exported function / method / type (in any language). Don't gate on \"missing tests\" — let `test-author` decide whether existing coverage is sufficient.",
    "   - `task` with subagent_type `validator` — confirm `make check` (or the repo's equivalent — Makefile / package.json / Cargo / go.mod) is green.",
