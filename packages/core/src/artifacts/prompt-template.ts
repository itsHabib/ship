...
export function renderImplementationPrompt(input: RenderImplementationPromptInput): string {
  ...
  return [
    ...
    "   Skip any that don't apply (e.g. test-author on a docs-only change). If the `task` tool's subagent_type enum only lists `generalPurpose | cursor-guide | best-of-n-runner` (no repo-registered subagents), skip this rule entirely and note the gap in the structured summary's blockers section. If `task` returns an error for an invocation you did attempt, write `task-error: <verbatim error message>` in the blockers section — do NOT fabricate subagent output.",
