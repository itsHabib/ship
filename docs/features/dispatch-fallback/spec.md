...
5. Chains are validated at import: unknown/illegal targets (per the `selectRunner` matrix), `rooms`, dupes, local targets on streams without `branch_name`, and **cloud targets on streams without `repo_url`** fail the import loudly, not at hop time.
   ...
   failure surfaces at EITHER seam:
     ...
     async: runToTerminal fails in background (setImmediate) → pollOneStream
            sees terminal failure with classifyFailure category
              │
              ▼
   eligible category AND no work products AND next viable target?
     transient+retryable, target unretried → one same-target re-dispatch
                                             (recorded {retried})
...
> eligible category **AND** stream carries no work products: no recorded branch push, `prUrl` empty, **`reviewCycles == 0`** (the #184 flip-skip lesson — `prUrl` alone lags reality).
...
- **Import time (structural — reject):** a `(local, *)` target on a stream with no `branch_name`, or a `(cloud, *)` target on a stream/manifest with no `repo_url` (cycle-2 finding: without this, a local-primary stream with a cloud fallback hops into a `cloud stream requires repo_url` preflight throw *outside* the fallback path). Both are authoring errors — the route could never work; reject the import.
- **Hop time (environmental — skip loudly):** required env credential absent, or local worktree missing → the target is skipped with a recorded reason and the chain advances. Env requirements are a **per-provider table, not a local-claude special case** (cycle-2 finding — the driver-hardening run's `MissingApiKeyError` was a *cursor* cell): `cursor` → `CURSOR_API_KEY`; `claude` (local or cloud) → `ANTHROPIC_AUTH_TOKEN` or `ANTHROPIC_API_KEY`; `codex` → its key per `default-wiring.ts`. The table lives next to the `selectRunner` wiring, which already knows each runner's construction needs.
...
A single 10-second `api.cursor.com` connect-timeout (friction log 2026-07-06 F1 — literally `isRetryable: true`) must not permanently demote a stream off its intended target: "each target once" + "chains don't refill" would otherwise turn a blip into a lifecycle decision. Rule: when the failure is marked retryable by the runner **and** the current target hasn't been retried yet, re-dispatch the same target once — transparently, recorded in `fallbackLog` as `{retried, target, category, at}` — before the cursor advances. One retry per target per lifecycle; second failure advances the chain. Non-retryable failures advance immediately. This stays inside the #199 breaker budget (§7.2 step 0: two attempts on a target, then movement — the breaker's 3-consecutive park is unreachable while the chain is live).
...
- **Manifest keys** as in §5; import validation extends `collectStreamPreflightErrors`: every chain target must be a wired `selectRunner` cell, not `rooms`, not a dupe of the primary or an earlier entry, `(local, *)` requires `branch_name`, `(cloud, *)` requires `repo_url`.
...
2. Validate every target (wired cell, no `rooms`, no dupes vs primary/earlier entries, `(local, *)` requires `branch_name`, `(cloud, *)` requires `repo_url`) → structured import failure on violation.
3. If any chain target's per-provider env requirement (§4.4 table) is unmet in the current env: record a run-level warning (visible in status), proceed.
...
2. Gate: category in the eligibility allowlist **AND** no work products (no recorded branch push, `prUrl` empty, `reviewCycles == 0`). Fails the gate → step 7.
3. **Transient retry (§4.7):** failure marked retryable and current target unretried → append `{retried}` record, reset to pending on the *same* target, done (re-dispatch next tick). Otherwise continue.
