...
**D5 — No shared control-plane database; GitHub is the cross-seat rendezvous.** The driver store stays seat-local SQLite. A run belongs to the seat that started it; what any seat (and the operator) can always see is the external truth — branches, PRs, reviews, checks on GitHub. Seat handoff is `driver render`/`status` JSON + GitHub state, not database replication. Escalation rows are seat-local too, so their visibility semantics are explicit rather than implied: **the row is the durable guarantee, delivery is the outward one.** Page-class rows push outward at write time via the notify hook; queue-class rows are read by the digest step on the seat's own machine. For a dead *local* overnight seat that is the operator's machine — nothing strands. For a *cloud* seat, unresolved escalation rows and recording intents ride the same relay export (§7 Flow E), and the page-class push has already left the box; a cloud seat that dies between a park and its notify loses at most one tick's delivery, and the run's GitHub state remains inspectable — the accepted residual of refusing a shared database. *Alternative rejected:* a hosted store (managed Postgres or similar) — an infra phase, a new failure domain, and cloud-first when the guardrail says local-first. *Consequence that must be fixed first:* two seats on ONE machine already see different stores — the packaged-app connector resolves its state under the app-virtualized data dir while a terminal CLI resolves the real one, and the CLI resolvers ignore `SHIP_DB_PATH`/`SHIP_RUNS_DIR` that the MCP server honors. Store convergence (C7) makes "one machine = one store" true and is sequenced before any seat-portability claim (§9 Phase 2).
