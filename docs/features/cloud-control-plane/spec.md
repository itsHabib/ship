...
| Security | Seats hold scoped tokens only (agent-scoped GitHub token pattern); grant mutation is local-CLI-only, interactive-only (TTY + typed confirmation), and pages on every change; the gate never merges changes to the gate |
...
The residual is named, not hidden: a local seat runs as the operator's OS user, so the fence against it is friction + loudness, not cryptography — the durable authority root sits one layer down, in the **GitHub token**. A cloud seat's scoped token cannot `--admin`-merge at all (server-side enforcement independent of anything the seat runs); the local overnight seat necessarily holds the operator's token, which is exactly why L2's entry gate demands the shadow track record first and why the caps stay engine-enforced. The adversarial break-the-gate pass is a merge precondition for the C14 PR (fail-open history says the bot panel alone misses structural holes).
...
- **`escalations`** (new): `id`, `driver_run_id`, `stream_id?`, `class`, `payload_json`, `created_at`, `notified_at?`, `resolved_at?`, `resolution?`. Dedup key `(driver_run_id, COALESCE(stream_id, ''), class)` — the run-level sentinel is load-bearing: SQLite composite UNIQUE indexes never collide on NULL, so run-level classes (`spend-ceiling`, `pathological-batch`) would duplicate freely without it. Classes (initial set): `triage-uncertain`, `auth-rejection`, `cycle-exhausted`, `product-direction`, `sensitive-path`, `merge-blocked-no-verdict`, `spend-ceiling`, `pathological-batch`, `ci-infra`, `grant-mutated`.
...
**E — Cloud seat recording relay.** Cloud seat merges PRs → emits recording intents (task id, PR, merge sha) — and, on run end, its unresolved escalation rows — as a relay export on the run record → operator's next local session runs the digest step → replays intents into dossier (`task_complete` + `artifact_link`), idempotent on (task, sha), and folds the exported escalations into the morning queue → marks the export applied. Dossier never becomes a runtime dependency of the engine or the cloud seat.
