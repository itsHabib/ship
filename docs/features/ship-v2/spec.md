...
2. **02 — open_pr phase (removed).** Originally shipped a `Phase.kind = "open_pr"` + `open_pr` MCP tool. Walked back as ship's scope shrunk inward; PR creation moves to a future `gh` MCP shim. See [docs/features/remove-open-pr/spec.md](../remove-open-pr/spec.md). The `phaseKindSchema` retains `"open_pr"` as a tombstone for historical row hydration.
