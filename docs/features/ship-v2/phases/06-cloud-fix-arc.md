...
`CLOUD_SANDBOX_REPO_URL` in `e2e/scenarios/cloud-e2e-helpers.ts` and 5 sister references update from `ship-live-sandbox` (doesn't exist) to `agent-sandbox` (does exist). Verified via gh API earlier this session.
...
8. URL sweep — replace `ship-live-sandbox` with `agent-sandbox` in 6 files (see § Scope).
