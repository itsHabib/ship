...
- `engine.test.ts` (L2, rooms-CLI double via `createFakeShipPort`):
  ...
  - **preflight** — a rooms manifest without `repo_url` fails preflight; a rooms stream without `branch_name` (or a blank/whitespace one) fails preflight; a rooms stream with a non-cursor provider (`claude`) is rejected at preflight and nothing dispatches.
