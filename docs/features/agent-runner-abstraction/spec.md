...
**Update — v8 (2026-07-29, unattended permission contract):** local Claude execution now uses SDK `auto` mode exclusively. The obsolete `bypassPermissions` + dangerous-skip path is removed. Ship requests `auto` through both the query option and inline settings, requires the live `system/init` event to report `auto`, and fails closed on a stale, missing, or mismatched mode. Driver retry/fallback does not weaken or replace this runner-local invariant.
...
3. `ClaudeRunner.run` builds `query({ prompt, options: { cwd, model, env, permissionMode: "auto", settings: { permissions: { defaultMode: "auto" } }, mcpServers, agents, abortController } })`.
