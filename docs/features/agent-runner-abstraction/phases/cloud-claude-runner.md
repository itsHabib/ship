...
- **Agent** — `client.beta.agents.create({ model, name, system?, tools?, mcp_servers? })` → `{ id, version }`. `tools: [{ type: "agent_toolset_20260401", default_config: { enabled: true, permission_policy: <always-allow> }, configs: [] }]` enables bash/edit/read/write/glob/grep with **auto-approve**. This cloud tool policy is separate from local Claude execution, which uses SDK `auto` permission mode. `mcp_servers` (remote URL transport) carries the GitHub MCP in 3b. **Verify the exact `tools`/`permission_policy` param shapes against the installed types** (the always-allow policy type name + the toolset-params variant) — build the mock from the real types.
