...
4. **core / prompt-template** — parameterize the `Co-authored-by: Cursor <cursoragent@cursor.com>` trailer by provider: cursor emits the cursor trailer; other providers emit their own or none. **The trailer string appears TWICE** — [prompt-template.ts:45](../../../packages/core/src/artifacts/prompt-template.ts) (primary commit instruction) AND :55 (the subagent P0/P1 follow-up-commit instruction) — parameterize **both** (a line-jump to :45 alone leaves it hardcoded on the follow-up path); also update the assertion in `prompt-template.test.ts:42`. (Phase-1 scope: cursor-only, so the trailer is genuinely all the de-cursoring needed here. The deeper prompt-contract coupling — the Cursor `task`-tool enum + `Explore`/`Bash`/`Browser` subagent names baked into the rendered prompt — is a **Phase 2** concern, spec.md §9 2a.)
5. **store + workflow** — add a `provider` column to `cursor_runs` (default `'cursor'`). Concrete cross-package touchpoints (review — under-specified as just "store"): new migration `0007_cursor_runs_provider.sql` (`ALTER TABLE cursor_runs ADD COLUMN provider TEXT NOT NULL DEFAULT 'cursor'`); **the persisted run shape is the `.strict()` `cursorRunRefSchema` in `@ship/workflow`** (NOT just `store`) — a strict schema rejects an unknown `provider` key, so add the field there too; extend `CURSOR_RUN_COLUMNS`, the INSERT param list, `RecordCursorRunInput`, `CursorRunRow`, and `parseCursorRun` in `store/cursor-runs.ts`; and update `migrations.test.ts` (it asserts the exact migration-file list + count — adding `0007` breaks both by design).
