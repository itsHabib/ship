...
`preflightPool(pool, defaultRuntime, deps)` (in `assign.ts`, which already owns `PoolMember` + runtime resolution) resolves each member to its target (`member.runtime ?? defaultRuntime`), dedups targets on the full `(runtime, provider, model_id)` key, calls `checkTargetViability` once per unique target, and maps verdicts back to members — returning `{ effective, dropped }` in original pool order.

**Unprefixed-member runtime (design call, flagged for review).** A pool member without a runtime prefix inherits its stream's runtime at assignment, which is not known during the whole-pool preflight phase. Preflight resolves such a member against the manifest's `default_runtime` (`stream.runtime` is not yet bound to a member). For cursor this is immaterial (runtime-agnostic catalog check); for claude/codex it picks the credential arm from the manifest default. This matches the spec's "warning-grade, advisory" framing — preflight catches dead credentials and typo'd ids before the batch fires, not a per-stream runtime proof. Documented here so the panel can object if a stricter per-stream resolution is wanted.
...
- `viability.test.ts`: cursor id in/out of catalog; claude local present via `ANTHROPIC_AUTH_TOKEN`, present via `ANTHROPIC_API_KEY`, absent; claude cloud present/absent (only `ANTHROPIC_API_KEY` counts); codex present via either var, absent; the dedup path (two members differing only by runtime → one cursor fetch).
- `assign.test.ts` (extend): preflight drops a non-viable member → effective pool shrinks, dropped recorded in advisory; empty effective pool throws `AssignError` and writes nothing; `--no-preflight` path → effective == pool, dropped == []; `assigned_at` pinned via injected `now`; the 2a idempotence test updated to inject a fixed `now` and still round-trips.
- `make check` green on ubuntu + windows (typecheck + lint + format + coverage; driver branch threshold).

## Risks

- **R1 — cursor `/v1/models` shape/auth drift.** The adapter targets the same endpoint the runway grok run proved. If auth or the response shape differs, the adapter (CLI-edge) changes without touching the helper contract. Mitigated by keeping the parse in the adapter and the helper taking `string[]`.
