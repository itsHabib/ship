...
1. **Age floor.** The cap tracks the best provable *lower bound* on the run's total age: the max over the signals below. Each signal is a same-clock pair, so each is a true lower bound under its stated assumptions; the max is therefore also a lower bound. Floor ≥ cap ⇒ expire (`timeout-near-cap`).
...
| `streamAge` | latest event ts − earliest known run ts | provider server | stream dies with the local connection; hung run emits nothing |
...
- **Misfire** (timer fired before its armed delay elapsed in real time — the suspend/clock-jump detector from #165):
  1. Sync floor = max(`monoAge`, `streamAge`). If ≥ cap → expire now. No I/O on this path.
...
- **Cloud attach** (resume): the call site passes `kind: "attach"`; once the handle exists the cap fires one initial probe. If the server-anchored age already ≥ cap, shrink the window to the existing grace floor (`MIN_RESUMED_CAP_WINDOW_MS`) — an already-terminal run can still deliver its real result inside the grace; a still-running one gets the synthetic `timeout-near-cap` at its end. This closes the `startedAt`-missing / negative-delta hole where `resumeElapsedMs` returns 0 and today's code silently grants a full fresh window. The `kind` flag is required because the cap cannot distinguish "fresh dispatch" from "resume with a broken seed" by `elapsedMs === 0` alone.
...
The synthetic terminal reports the age floor that triggered expiry (always ≥ the configured cap), so `classifyFailure` still lands on `timeout-near-cap` deterministically.
...
- **Runners are mechanism** — they expose raw signals, no decisions. `AgentRunHandle` grows two optional members: a sync, I/O-free liveness snapshot fed by the runner's own event stream (`{ createdAtMs?, lastEventAtMs? }` — server-stamped, via each provider's `EventProjection.timestamp`), and a bounded async `probe()` returning the provider record's server-stamped `{ status?, createdAtMs?, updatedAtMs? }`. Cursor cloud implements the probe from the agents REST surface (`V1Run.createdAt/updatedAt`; the SDK `Run` object exposes `createdAt` and live `status` — implementation may use whichever SDK/REST path yields the server pair). Claude cloud implements it over the sessions API if it exposes server timestamps cheaply, else returns `undefined` — a documented degrade, not an error. Local and rooms runners expose neither member.
...
- A resume after the window has elapsed remotely gets at most the grace window, then cancels (`timeout-near-cap`) — including when the persisted seed is missing or insane. It is never handed a fresh full window.
...
- Attach with probe age ≥ cap → window shrinks to grace; a real terminal result inside the grace beats the synthetic; absent that, `timeout-near-cap`.
...
1. `agent-runner`: optional liveness/probe members + `AgentRunLiveness`/`AgentRunSnapshot` types; extend fakes with scriptable signals.
2. `cursor-runner` cloud: stream-fed liveness snapshot + REST/SDK probe.
3. `claude-runner` cloud: sessions-API probe, or `undefined` degrade with a doc note.
4. `duration-cap.ts`: floor composition, misfire decision, fail-closed counter, attach-time initial probe, `kind` arg.
5. `service.ts`: thread signals + `kind` through `runToTerminal` and `runResumeAttach`.
