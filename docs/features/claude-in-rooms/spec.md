...
>
> **v3 changelog (second-pass fold-in, codex + claude[bot]):** both reviewers confirmed the design is ready to proceed; v3 folds their precision refinements into the P1/P3 impl requirements (no design change). §4a — the policy gate must reject API-key **sources by provenance**, because `routePinnedCredential` launders a pinned `ANTHROPIC_API_KEY` into the `ANTHROPIC_AUTH_TOKEN` slot and would otherwise pass the assert; and made explicit that `buildRoomsEnv`'s allowlist is the load-bearing guard (`resolveDispatchCredential` only strips when a `.ship.json` `credentials` block is in scope). §4c — leak-scan uses a **canary credential and scans for the bearer *value***, not the variable name. §7 — the no-branch→`done` fix now specifies the `branches[0].branch` predicate, the `classifyLandedStreamTriage` short-circuit, and a **terminal ledger event** (else `run_finished` is rejected). No open decisions remain from review.
...
- **`buildRoomsEnv` must build an explicit allowlist, not copy `process.env`.** It carries all of the host env today (`{ ...process.env }`); `sudo -E` forwards the whole calling environment, so an operator dotfile that sets `ANTHROPIC_API_KEY` would reach the guest. The claude path must construct an allowlisted guest env **and assert `ANTHROPIC_API_KEY` is absent** (strip explicitly + assert, not merely omit). **This allowlist is the load-bearing protection in *all* cases** — `resolveDispatchCredential` only strips/routes when a `.ship.json` `credentials` block is in scope; with no such block it returns the env unchanged. Do **not** read the spec as "`resolveDispatchCredential` strips `ANTHROPIC_API_KEY` unconditionally" — it does not; the two layers are complementary (allowlist = the guard, credential policy = pin/`forbid_env` enforcement on top).
- **Preserve credential provenance through the gate — reject API-key *sources*, don't infer viability from the routed env.** Sharp interaction codex flagged: when a repo pins `credentials.claude_token_env` to `ANTHROPIC_API_KEY` (or any API-key-bearing slot), `resolveDispatchCredential` → `routePinnedCredential` **strips the original slot and moves the value into `ANTHROPIC_AUTH_TOKEN`**. The rooms allowlist + `ANTHROPIC_API_KEY`-absent assert would then *pass* on a laundered API key sitting in the OAuth slot — exactly the API-key-only dispatch the viability check is supposed to reject (and it would hard-fail the CLI mid-run, no credit). So the "reject API-key-only" preflight (§4a first bullet) must key off the **original credential kind/provenance the policy gate returns**, not the post-routing env shape. P3: the credential policy must surface provenance and reject API-key sources for the rooms cell.
...
**(c) Leak-verification is a firm P1 exit criterion, not a soft flag — and it scans for the bearer *value*, not its variable name.** codex's refinement: grepping for the string `CLAUDE_CODE_OAUTH_TOKEN` passes even if `claude -p`, a cache, or the task writes the raw bearer *value* to a workspace file under a different (or no) label — and that file can still enter the `--out` tar or the pushed commit. So the spike must run with a **known canary credential** and scan for **that exact secret value** across:
- `/workspace/**` (nothing the `--out` tar or a commit could pick up),
- the collected `--out` artifacts,
- the resulting commit / pushed branch,
- `result.json`.

Also confirm `claude -p` print mode writes no auth/session cache under `/workspace/**` (the tar + push boundary).
...
**No-changes path (⚠ requires a state-machine fix — codex P1):** `claude -p` makes no edits → no commit → rooms reports no `pushed_branch` (the `refs/rooms/base` no-changes detection already built for cursor). The driver does **not** correctly handle this today: `handleSucceededPoll` unconditionally applies `buildLandedPatch` (and then runs `classifyLandedStreamTriage`), and `isStreamDoneOrSkipped` excludes `landed`, so `isBlockedOnMerges` treats every `landed` stream as awaiting a merge — a no-branch success sticks in `blocked_on_merges` and can stall dependent batches instead of resolving. **P3 must add an explicit success-without-branch → `done` transition, with tests.** The v2 note under-specified it; P3 must cover all three layers:
1. **Predicate:** the check is on `wfRun.branches?.[0]?.branch` being absent (what `buildPrMetaPatch` reads) — *not* `result.json` directly. A succeeded stream with no branch is done-without-PR.
2. **Scheduler:** move the stream directly to `"done"` so `isStreamDoneOrSkipped`/`everyStreamTerminalDoneOrSkipped` see it as terminal and `evaluateExit` returns `{ status: "done" }`. **Short-circuit before `classifyLandedStreamTriage`** — there is no PR to triage; running it on a stream with no `prNumber` no-ops or logs a confusing failure. So the no-branch path skips *both* `buildLandedPatch` and the triage call, not just the patch.
3. **Ledger (codex P1):** the append-only driver-state ledger must also get a terminal event. Today `driverstate-emit.ts` emits terminal attempts only for `landed`/`failed` and nothing for `done`, while the emitter permits `run_finished` only once every stream is `merged`/`skipped`/`failed` — so a no-change run would remain `dispatched`/`landed` in the ledger and its `run_finished` receipt would be **rejected**. P3 must define + emit a terminal **no-change event/transition** in the ledger, not only flip the driver-store status.

This is a real driver change, not just a claude-runner concern — it was latent on the cursor path too, but the claude/rooms gate is the first place it's exercised.
...
| **P3 — ship RoomClaudeRunner + legal cell + driver fix** | ship dispatches claude/rooms | `RoomClaudeRunner` + `buildRoomsArgs` claude variant routed through the `resolveDispatchCredential` chokepoint (§4a — not around it by package placement); rooms-specific OAuth viability in preflight that rejects API-key **sources** by provenance (§4a — not by post-routing env shape); allowlisted `buildRoomsEnv` + `ANTHROPIC_API_KEY`-absent assert; `selectRunner` + `roomClaude` wiring; `isLegalCell`/preflight/MCP-refinement make `(claude,rooms)` legal; **no-branch-success → `done`: predicate on `branches[0].branch`, skip `classifyLandedStreamTriage`, + terminal ledger event (§7) + tests**; unit tests (routing + legality) | P2 (impl); may build + review against a rooms-CLI mock **concurrently with P2** once §6's `rooms run --runner claude` interface is frozen | — |
